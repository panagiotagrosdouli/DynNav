...
DynNav investigates deterministic, grid-based navigation and rerouting under occupancy uncertainty, risk, recoverability constraints, and mission-level supervision. The documentation must be read together with the executable Python package and generated evidence. No formal safety guarantee, production Nav2 plugin, Gazebo validation, TurtleBot validation, or hardware result is claimed by this index.

## Documentation map

- [`REPOSITORY_AUDIT.md`](REPOSITORY_AUDIT.md): current capabilities, evidence gaps, severity-ranked findings, and implementation order.
- [`RESEARCH_OVERVIEW.md`](RESEARCH_OVERVIEW.md): research question, motivation, and scope.
...
- **Implemented:** code exists and is exercised by tests or CI.
- **Research Prototype:** executable or partially tested, but not deployment-ready.
...
See the [root README](../README.md), the [`configs` guide](../configs/README.md), the [`scripts` guide](../scripts/README.md), and the [`tests` guide](../tests/README.md).
