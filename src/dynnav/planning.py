...
class NavigationPolicy:
    ...
    def plan(self, grid: GridMap, start: Pose, goal: Pose) -> tuple[Trajectory, PlanningMetrics]:
...
class RiskAwareAStar(NavigationPolicy):
    ...
    def plan(self, grid: GridMap, start: Pose, goal: Pose) -> tuple[Trajectory, PlanningMetrics]:
        ...
        while frontier and expanded < self.max_expansions:
            ...
            for neighbor in grid.neighbors4(current):
                ...
                if occupancy >= 0.98:
                    continue
                returnability_penalty = self._returnability_penalty(grid, neighbor, start)
                ...
                if neighbor not in cost_so_far or new_cost < cost_so_far[neighbor]:
                    ...
                    heapq.heappush(frontier, (priority, expanded, neighbor))
    ...
    def _returnability_penalty(self, grid: GridMap, pose: Pose, start: Pose) -> float:
        local_degree = sum(1 for n in grid.neighbors4(pose) if grid.probability(n) < 0.65)
...
class RecoverabilityEstimator:
    ...
    def score(self, grid: GridMap, path: list[Pose], origin: Pose) -> float:
        ...
        if not path:
            return 0.0
        planner = RiskAwareAStar(risk_weight=1.0, returnability_weight=0.0, max_expansions=3000)
        successes = 0
        for pose in path[:: max(1, len(path) // 8)]:
            trajectory, metrics = planner.plan(grid, pose, origin)
            if metrics.path_found and trajectory.risk < 0.65:
                successes += 1
        samples = len(path[:: max(1, len(path) // 8)])
        return successes / max(samples, 1)
...
class DynamicRerouter:
    ...
    def maybe_replan(self, grid: GridMap, current: Pose, goal: Pose, active: Trajectory) -> tuple[Trajectory, bool]:
        """Return an updated trajectory and whether replanning occurred."""
        if active.risk <= self.risk_threshold and active.recoverability >= self.recoverability_threshold:
            return active, False
...
class PlannerSwitch:
    ...
    def select(self, estimated_risk: float) -> NavigationPolicy:
        ...
        return self.conservative if estimated_risk >= self.risk_threshold else self.nominal
