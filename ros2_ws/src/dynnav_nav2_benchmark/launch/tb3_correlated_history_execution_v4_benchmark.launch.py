"""Launch G1 V4 with controlled odometry localization and no AMCL."""
...
def _launch_setup(context):
    ...
    spawn_robot = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            str(sim_share / "launch" / "spawn_tb3.launch.py")
        ),
        launch_arguments={
            "namespace": "",
    ...
    cleanup_world = RegisterEventHandler(
        OnShutdown(on_shutdown=[OpaqueFunction(function=_remove_world)])
    )
        ...
        [
            ...
            DeclareLaunchArgument(
                ...
                default_value=str(
                    benchmark_share
                    / "worlds"
                    / "g1_parallel_corridor.sdf.xacro"
                ),
            ),
            DeclareLaunchArgument(
                "blocker_sdf",
            ...
            DeclareLaunchArgument(
                "output_dir",
                default_value="/tmp/dynnav_g1_v4",
            ),
            DeclareLaunchArgument("repetitions", default_value="1"),
            DeclareLaunchArgument(
                "dependence",
                default_value="",
                description="Optional frozen dependence slice.",
            ),
