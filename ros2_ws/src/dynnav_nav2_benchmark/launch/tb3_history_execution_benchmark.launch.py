"""Launch frozen action-triggered history execution trials in Gazebo/Nav2."""
...
from dynnav_nav2_benchmark.history_execution import load_history_execution_suite, transition_from_world_trigger, world_to_cell
...
def _launch_setup(context):
    ...
    trigger = transition_from_world_trigger(
        suite.scenario.trigger,
        origin_x=origin_x,
        origin_y=origin_y,
        resolution=resolution,
    )
    safe_cell = world_to_cell(
        suite.scenario.safe_region.center,
        origin_x=origin_x,
        origin_y=origin_y,
        resolution=resolution,
    )
    closure_cell = world_to_cell(
        Pose2D(suite.scenario.blocker_pose.x, suite.scenario.blocker_pose.y),
        origin_x=origin_x,
        origin_y=origin_y,
        resolution=resolution,
    )
    if trigger != ((174, 189), (175, 189)) or safe_cell != (160, 190) or closure_cell != (181, 191):
        raise RuntimeError(
            "map geometry does not match the frozen action-trigger protocol: "
            f"trigger={trigger}, safe={safe_cell}, closure={closure_cell}"
        )
    ...
    payload = inject_history_planner_parameters(
        ...
        closure_probability=suite.scenario.trigger.closure_probability,
        recoverability_weight=suite.scenario.recoverability_weight,
    )
    ...
    shutdown = RegisterEventHandler(
        OnProcessExit(
            ...
            on_exit=[EmitEvent(event=Shutdown(reason="DynNav history benchmark completed"))],
        )
    )
    return [simulation, bridge, delayed_runner, shutdown]
...
def generate_launch_description() -> LaunchDescription:
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument("generated_params_file", default_value="/tmp/dynnav_history_nav2_params.yaml"),
            DeclareLaunchArgument("scenario_file", default_value=str(benchmark_share / "config" / "sandbox_history_triggered_events.yaml")),
            DeclareLaunchArgument("map_file", default_value=str(nav2_share / "maps" / "tb3_sandbox.yaml")),
            DeclareLaunchArgument("blocker_sdf", default_value=str(benchmark_share / "models" / "dynamic_blocker.sdf")),
            DeclareLaunchArgument("output_dir", default_value="/tmp/dynnav_history_benchmark"),
