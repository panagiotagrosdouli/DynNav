...
\begin{abstract}
A mobile robot can reach the same geometric location through two paths while retaining different future return options if its earlier actions alter the probability of later environmental closures. A state-only navigation model cannot represent this distinction. We study action-triggered topology hazards: traversing selected directed transitions activates stochastic future closures of cells that may be critical for returning to a designated safe set. We formulate safe-return reliability on an augmented state consisting of robot position and activated hazards, give an exact small-world oracle under independent Bernoulli closures, and plan with A* using a soft recoverability cost. A counterfactual benchmark shows that histories ending at the same cell can differ in return reliability by as much as 0.9 while a state-only marginal-risk model necessarily assigns them the same value. In an analytic route-switch family, the exact planner matches all 75 predicted switch decisions. In paired stochastic execution over 1000 seeds per condition, shortest-path and state-only marginal-risk planning exhibit identical irreversible-failure rates of 0.512, 0.881, and 0.992 at closure probabilities 0.2, 0.5, and 0.8, whereas exact history-aware planning avoids all trigger activations in this controlled family. A frozen heterogeneous-probability evaluation over 1500 additional paired trials preserves the state-only null result and separates the soft history-aware planner from a fixed hard-return threshold in one scenario. A structural critical-cut approximation substantially reduces return-oracle latency on a series-critical family, but a joint-cut counterexample exposes its optimism. The results isolate a specific information advantage of history conditioning while making explicit the limits of the current discrete, known-model evaluation.
\end{abstract}
...
\begin{itemize}
  ...
  \item analytic, procedural, stochastic, scaling, frozen heterogeneous-probability, and adversarial benchmarks against shortest-path, state-only probability-aware, critical-cut, and hard safe-return baselines;
...
\begin{enumerate}
  ...
  \item \emph{Approximation boundary:} can a joint cutset produce a route-history failure for the cut approximation?
\end{enumerate}

The development sweeps use homogeneous closure probabilities. Before inspecting outcomes, a separate evaluation family was frozen with heterogeneous per-trigger probabilities and unseen module counts: six modules with probabilities $(.15,.25,.35,.55,.75,.85)$, seven modules with $(.80,.20,.70,.30,.60,.40,.90)$, and eight modules with $(.10,.90,.20,.80,.35,.65,.45,.75)$. Each scenario uses 500 paired execution seeds. This protocol tests probability-pattern and horizon generalization within the same procedural topology family; it is not claimed as geometric-domain generalization.
...
The state-only null result is more informative than comparison only against shortest path. Both planners know the same closure locations and marginal probabilities. Their divergence arises because one conditions on the executed trigger set and the other does not. This distinction survives the frozen heterogeneous probability patterns and longer horizons tested here. However, those scenarios preserve the same repeated-module topology, so they test parameter/horizon generalization rather than transfer to arbitrary maps.

The hard safe-return baseline matches exact-history planning in the homogeneous high-penalty series experiment but differs in the eight-module heterogeneous case. That difference should not be read as a universal advantage of a soft objective: a different threshold can change the hard-constrained policy. The result instead shows that ``feasible above a threshold'' and cumulative return-fragility optimization answer different decision questions.
...
The strongest stochastic results come from procedural worlds designed to expose a mechanism. The homogeneous and heterogeneous series families contain critical return bridges, so a policy that avoids every trigger has a structural advantage once closure probabilities become appreciable. The state-only baseline is intentionally given the same marginal closure probabilities but an incorrect conditional model; its failure demonstrates the information loss under that modeling choice, not the weakness of every possible Markov representation. A sufficiently rich Markov state could include the activated-hazard set, which is exactly our augmented state.

Parameter selection is another threat. The principal soft-planner execution results use recoverability weight $\lambda=8$, while the hard baseline uses $\tau=0.9$. Analytic phase-boundary sweeps and threshold-sensitivity experiments expose how route choice changes with these parameters, but no claim of universal parameter optimality is made. Finally, Monte Carlo uncertainty remains in empirical failure rates; paired binary effects and exact McNemar tests are used because seeds are shared across planners.
...
The implementation records raw per-trial CSV files and machine-readable JSON summaries for every benchmark family. Focused continuous integration runs regression tests before executing the benchmark suite, then uploads all raw artifacts under the exact commit SHA. Planning-time instrumentation excludes post-hoc diagnostic profiling while retaining oracle work used online for route selection. Stochastic comparisons use fixed paired seeds and event-index common random numbers. These controls were introduced before the frozen heterogeneous evaluation was inspected. A machine-readable paper evidence manifest records the exact evidence-producing workflow run, artifact identity, metric values, and claim scope.

\section{Conclusion}
We studied navigation in which the robot's own transition history activates stochastic future closures that can destroy return connectivity. In this setting, the same geometric cell can correspond to different recoverability because the environment's future topology depends on how the robot arrived. Augmenting search with the activated hazard set captures this distinction; a state-only marginal-risk baseline cannot. Exact planning follows an analytic route-switch boundary and eliminates failures in the controlled series families evaluated here, while a scalable critical-cut approximation reduces computation in series-critical cases but fails on joint cutsets. These results establish a reproducible mechanism-level case for history-conditioned safe-return reasoning and clearly delimit the additional evidence required for broader robotics claims.
