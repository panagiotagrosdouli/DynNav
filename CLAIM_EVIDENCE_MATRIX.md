...
| Exact state-only fixed-field baseline isolates representation from estimator choice | implementation added; same exact connectivity oracle and additive objective as history-conditioned planner, but no trigger conditioning | PENDING EVIDENCE | reviewer P0 workflow must retain outcomes before publication-facing claim |
| History-conditioned planning remains informative when every feasible route activates hazards | frozen 24-scenario procedural challenge protocol, seed 20260925, 500 paired seeds/scenario | PENDING EVIDENCE | no outcome claim until retained reviewer-P0 artifact exists |
