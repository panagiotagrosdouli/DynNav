...
| Recoverability probabilities are calibrated to real-world recovery success | synthetic/model probabilities plus controlled calibration/misspecification studies only | UNSUPPORTED | current G2/G3 results characterize model calibration boundaries but do not calibrate to physical-world recovery success |
| Strict exposure-gated online calibration can always learn a trigger-conditioned closure probability safely | cold-start lockout proposition and zero-exposure target-only replications disprove this | UNSUPPORTED | information must come from admitted exposure risk, side information, transfer, or another declared source |
| Exact shared-parameter side information can break the G3 target-exposure lockout | 10-seed sentinel-transfer grid reduces mean probability error from 0.225 target-only to 0.0094 under the exact shared-parameter condition | SUPPORTED | assumption-specific mechanism; optimistic transfer misspecification creates severe false-safe exposure |
| Logged-propensity IPW can validate randomized trigger-to-closure effects with adequate support | randomized graph stress grid: at 1000 samples/pair minimum mean recall 0.933 and precision 0.96 across the frozen propensity/effect grid | SUPPORTED | randomized/ignorable assignments with known propensities; not passive-log causal discovery |
| Exact closure-event quotient can remove large reachable trigger-history redundancy | duplicate-trigger diamond family: 398,583 raw reachable states vs 207 quotient states at 8 modules; exactness tests preserve return/path semantics | SUPPORTED | family/semantics-specific representation result; early-goal A* can realize much smaller savings |
| Exact history quotient universally yields large online planning speedups | early-goal A* benchmark shows only modest realized node/time reduction despite large full-state compression | UNSUPPORTED | search workload determines realized computational benefit |
