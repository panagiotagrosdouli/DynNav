...
| Repeated timing distributions characterize retained-run computational cost | 100 measured repetitions after 10 warm-ups in the pinned retained publication timing artifact: 12-hazard exact oracle median 34.51 ms vs critical-cut 0.170 ms; six-module history-exact median 5.16 ms, history-cut 3.65 ms, shortest 0.102 ms | SUPPORTED | runner-specific descriptive timing only; no hardware-independent runtime guarantee |
...
| History-conditioned planner improves Gazebo execution outcomes | strict retained reruns show 24/24 valid trials and 8/8 navigation success per planner, but the DynNavHistory-vs-DynNavShortest trigger-exposure difference varies between zero and one trial across reruns; release-candidate probe has no operational irreversible failures for either DynNav condition | UNSUPPORTED | retain Gazebo as execution/integration evidence only |
