...
| History-conditioned planner improves Gazebo execution outcomes | strict reruns do not show a stable effect: one retained run has trigger exposure 8/8 shortest vs 7/8 history, while the later exact-head rerun is 8/8 vs 8/8 with 7/8 realized closures for both and no irreversible failures | UNSUPPORTED | retain Gazebo as execution/integration evidence only |
