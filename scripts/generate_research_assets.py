...
def write_diagram(path: Path, title: str, nodes: list[str]) -> None:
    ...
    for idx, node in enumerate(nodes):
        ax.text(idx, 0.5, node, ha="center", va="center", bbox={"boxstyle": "round", "fc": "white"})
        if idx < len(nodes) - 1:
            ax.annotate("", xy=(idx + 0.42, 0.5), xytext=(idx + 0.58, 0.5), arrowprops={"arrowstyle": "->"})
...
def main() -> None:
    ...
    write_diagram(Path("assets/architecture.png"), "DynNav architecture", ["Perception", "Belief", "Risk Planner", "Monitor", "Control"])
    write_diagram(Path("assets/navigation_pipeline.png"), "Navigation pipeline", ["Sense", "Propagate", "Plan", "Monitor", "Reroute"])
